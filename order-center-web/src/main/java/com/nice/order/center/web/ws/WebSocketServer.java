...
public class WebSocketServer {
    ...
    public void onOpen(@PathParam("sid") String sid, Session session) {
        ...
        // 加入 Map中。将页面的sid和session绑定或者session.getId()与session绑定
        // ONLINE_SESSION_CLIENT_MAP.put(session.getId(), session);
        ...
        sendToOne(sid, "连接成功");
        log.info("连接建立成功，当前在线数为：{} ==> 开始监听新连接：session_id = {}， sid = {},。", ONLINE_SESSION_CLIENT_COUNT,
                session.getId(), sid);
    }
    ...
    public void onClose(@PathParam("sid") String sid, Session session) {
        // ONLINE_SESSION_CLIENT_MAP.remove(session.getId());
        // 从 Map中移除
        ONLINE_SESSION_CLIENT_MAP.remove(sid);

        //在线数减1
        ONLINE_SESSION_CLIENT_COUNT.decrementAndGet();
        log.info("连接关闭成功，当前在线数为：{} ==> 关闭该连接信息：session_id = {}， sid = {},。", ONLINE_SESSION_CLIENT_COUNT,
                session.getId(), sid);
    }

    /**
     * 收到客户端消息后调用的方法。由前端 <code>socket.send</code> 触发
     * 当服务端执行 toSession.getAsyncRemote().sendText(xxx) 后，前端的 socket.onmessage 得到监听。
     *
     * @param message
     * @param session
     */
    ...
    public void onMessage(String message, Session session) {
         ...
         */
        MessageReqDTO messageReq = new MessageReqDTO();
        messageReq.setMsg(message);
        try {
            messageReq = JacksonUtils.jsonToObject(message, MessageReqDTO.class);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            // ignore
        }
        String toSid = messageReq.getSid();
    ...
    private void sendToOne(String toSid, String message) {
        ...
        try {
            ...
            log.error("发送消息失败，WebSocket IO异常", e);
        }*/
    }
